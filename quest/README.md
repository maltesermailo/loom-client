...
### Test bitstream (M3.2, offline decode)

The decoder is fed from a raw Annex-B `.hevc` in the app's external files dir. It
is a §5-conformant synthetic stream generated by the host — regenerate and push
with:

```sh
# host repo: encode the synthetic pattern offline (no network)
loomd --insecure-dev --dump-hevc /tmp/loom_test.hevc --dump-frames 600

# push to the app's external files dir (adb can write there for a debuggable app)
adb shell mkdir -p /sdcard/Android/data/com.loom.quest/files
adb push /tmp/loom_test.hevc /sdcard/Android/data/com.loom.quest/files/loom_test.hevc
```

Absent → the app shows the static test image instead of failing. The `.hevc`
(~47 MB at 1440p) is not committed; it is regenerated on demand.

**Loopback measurement note:** on a desk the proximity sensor pauses rendering,
which stalls the decode consumer and skews latency numbers. Disable it via MQDH
(Developer settings → disable proximity/boundary) while measuring, and re-enable
for normal use.

### Logs

Everything the app logs uses the `loom` tag (including the R5 decode-latency
distribution once ~1000 frames have decoded):
