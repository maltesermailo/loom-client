...
QuicTransport::QuicTransport() {
  ...
  const QUIC_REGISTRATION_CONFIG reg = {"loom-quest", QUIC_EXECUTION_PROFILE_LOW_LATENCY};
...
bool QuicTransport::open_configuration() {
  QUIC_BUFFER alpn;
...
bool QuicTransport::start(const std::string& host, std::uint16_t port) {
...
void QuicTransport::send_control(std::span<const std::uint8_t> frame) {
...
void QuicTransport::close(std::uint64_t code) {
  if (connection_ != nullptr) {
    api_->ConnectionShutdown(connection_, QUIC_CONNECTION_SHUTDOWN_FLAG_NONE, code);
  }
}

std::optional<TransportEvent> QuicTransport::next_event() {
...
void QuicTransport::push(TransportEvent ev) {
  std::lock_guard<std::mutex> lock(mu_);
  events_.push(std::move(ev));
}

QUIC_STATUS QuicTransport::on_connection(HQUIC conn, QUIC_CONNECTION_EVENT* ev) {
  switch (ev->Type) {
...
QUIC_STATUS QuicTransport::on_stream(HQUIC /*stream*/, QUIC_STREAM_EVENT* ev) {
  switch (ev->Type) {
...
QUIC_STATUS QUIC_API QuicTransport::conn_trampoline(HQUIC conn, void* ctx,
                                                    QUIC_CONNECTION_EVENT* ev) {
  return static_cast<QuicTransport*>(ctx)->on_connection(conn, ev);
}

QUIC_STATUS QUIC_API QuicTransport::stream_trampoline(HQUIC stream, void* ctx,
                                                      QUIC_STREAM_EVENT* ev) {
  return static_cast<QuicTransport*>(ctx)->on_stream(stream, ev);
}
